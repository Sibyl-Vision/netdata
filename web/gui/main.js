...
var urlOptions = {
  ...
  genHash: function(forReload) {
    ...
    if (urlOptions.pan_and_zoom === true) {
      hash +=
        ";after=" +
        urlOptions.after.toString() +
        ";before=" +
        urlOptions.before.toString();
    }

    if (urlOptions.highlight === true) {
      hash +=
        ";highlight_after=" +
        urlOptions.highlight_after.toString() +
        ";highlight_before=" +
        urlOptions.highlight_before.toString();
    }
  ...
  netdataHighlightCallback: function(status, after, before) {
    ...
    if (status === true && after > 0 && before > 0 && after < before) {
      ...
      document.getElementById("navbar-highlight-content").innerHTML =
        ...
        d1 +
        " <code>" +
        NETDATA.dateTime.localeTimeString(after) +
        "</code></b> to " +
        " <b>" +
        d2 +
        " <code>" +
        NETDATA.dateTime.localeTimeString(before) +
        "</code></b>, " +
        "duration <b>" +
        NETDATA.seconds4human(Math.round((before - after) / 1000)) +
        "</b>" +
        "</span>" +
...
function renderStreamedHosts(options) {
  ...
  var base =
    document.location.origin.toString() + document.location.pathname.toString();
  if (base.endsWith("/host/" + options.hostname + "/")) {
    base = base.substring(
      0,
      base.length - ("/host/" + options.hostname + "/").toString().length
    );
  }
...
function renderMachines(machinesArray) {
  ...
  if (machinesArray === null) {
    ...
    if (ret) {
      ...
      console.log(
        "failed to contact the registry - loaded registry data from browser local storage"
      );
    }
  }
  ...
  if (machinesArray) {
    ...
    for (var machine of machines) {
      ...
      const alternateUrlItems = `<div class="agent-alternate-urls agent-${
        machine.guid
      } collapsed">
                ${machine.alternate_urls.reduce((str, url) => {
                  ...
                  return (
                    ...
                    `<div class="agent-item agent-item--alternate">
                                <div></div>
                                <a href="${url}" title="${url}">${truncateString(
                      url,
                      64
                    )}</a>
                                <a href="#" onclick="deleteRegistryModalHandler('${
                                  machine.guid
                                }', '${
                      machine.name
                    }', '${url}'); return false;">
      ...
      html += `<div class="agent-item agent-${machine.guid}">
                    <i class="fas fa-chart-bar" color: #fff"></i>
                    <span class="__title" onClick="return gotoServerModalHandler('${
                      machine.guid
                    }');">
                        <a class="registry_link" href="${machine.url}#">${
        machine.name
      }</a>
                    </span>
                    <a href="#" onClick="toggleAgentItem(event, '${
                      machine.guid
                    }');">
  ...
  if (!found) {
    ...
    const demoServers = [
      {
        url: "//london.netdata.rocks/default.html",
        title: "UK - London (DigitalOcean.com)"
      },
      {
        url: "//newyork.netdata.rocks/default.html",
        title: "US - New York (DigitalOcean.com)"
      },
      {
        url: "//sanfrancisco.netdata.rocks/default.html",
        title: "US - San Francisco (DigitalOcean.com)"
      },
      {
        url: "//atlanta.netdata.rocks/default.html",
        title: "US - Atlanta (CDN77.com)"
      },
      {
        url: "//frankfurt.netdata.rocks/default.html",
        title: "Germany - Frankfurt (DigitalOcean.com)"
      },
      {
        url: "//toronto.netdata.rocks/default.html",
        title: "Canada - Toronto (DigitalOcean.com)"
      },
      {
        url: "//singapore.netdata.rocks/default.html",
        title: "Japan - Singapore (DigitalOcean.com)"
      },
      {
        url: "//bangalore.netdata.rocks/default.html",
        title: "India - Bangalore (DigitalOcean.com)"
      }
    ];
...
function renderMyNetdataMenu(machinesArray) {
  ...
  if (options.hosts.length > 1) {
    html += `<div id="my-netdata-menu-streamed">${renderStreamedHosts(
      options
    )}</div><hr />`;
  }

  if (isSignedIn() || NETDATA.registry.isRegistryEnabled()) {
    html += `<div id="my-netdata-menu-machines">${renderMachines(
      machinesArray
    )}</div><hr />`;
  }
...
function isdemo() {
  ...
  try {
    if (typeof document.location.hostname === "string") {
      if (
        document.location.hostname.endsWith(".my-netdata.io") ||
...
function gotoServerValidateUrl(id, guid, url) {
  ...
  if (
    document.location.toString().startsWith("http://") &&
    ...
    document.location.toString().startsWith("https://") &&
  ...
  setTimeout(function() {
    document.getElementById("gotoServerList").innerHTML +=
      '<tr><td style="padding-left: 20px;"><a href="' +
      verifyURL(finalURL) +
      '" target="_blank">' +
      escapeUserInputHTML(url) +
      '</a></td><td style="padding-left: 30px;"><code id="' +
      guid +
      "-" +
      id +
      '-status">checking...</code></td></tr>';

    NETDATA.registry.hello(url, function(data) {
      if (
        typeof data !== "undefined" &&
        data !== null &&
        typeof data.machine_guid === "string" &&
        ...
        if (!gotoServerStop) {
          ...
          if (gotoServerMiddleClick) {
            ...
            document.getElementById("gotoServerResponse").innerHTML =
              "<b>Opening new window to " +
              NETDATA.registry.machines[guid].name +
              '<br/><a href="' +
              verifyURL(finalURL) +
              '">' +
              escapeUserInputHTML(url) +
              "</a></b><br/>(check your pop-up blocker if it fails)";
          } else {
            document.getElementById("gotoServerResponse").innerHTML +=
              "found it! It is at:<br/><small>" +
              escapeUserInputHTML(url) +
              "</small>";
        ...
        if (gotoServerValidateRemaining <= 0) {
          gotoServerMiddleClick = false;
          document.getElementById("gotoServerResponse").innerHTML =
            "<b>Sorry! I cannot find any operational URL for this server</b>";
        }
      }
    });
  }, id * 50 + penalty);
...
function gotoServerModalHandler(guid) {
  ...
  document.getElementById("gotoServerResponse").innerHTML = "";
  document.getElementById("gotoServerList").innerHTML = "";
  document.getElementById("gotoServerName").innerHTML =
    NETDATA.registry.machines[guid].name;
  ...
  if (!isSignedIn()) {
    ...
    setTimeout(function() {
      if (gotoServerStop === false) {
        document.getElementById("gotoServerResponse").innerHTML =
          "<b>Added all the known URLs for this machine.</b>";
...
function switchRegistryModalHandler() {
  document.getElementById("switchRegistryPersonGUID").value =
    NETDATA.registry.person_guid;
  document.getElementById("switchRegistryURL").innerHTML =
  ...
  if (n !== "" && n.length === 36) {
    NETDATA.registry.switch(n, function(result) {
      if (result !== null) {
        ...
        document.getElementById("switchRegistryResponse").innerHTML =
          "<b>Sorry! The registry rejected your request.</b>";
      }
    });
  } else {
    document.getElementById("switchRegistryResponse").innerHTML =
      "<b>The ID you have entered is not a GUID.</b>";
  }
}
...
function notifyForDeleteRegistry() {
  ...
  if (deleteRegistryUrl) {
    if (isSignedIn()) {
      deleteCloudAgentURL(deleteRegistryGuid, deleteRegistryUrl).then(
        (count) => {
          if (!count) {
            responseEl.innerHTML =
              "<b>Sorry, this command was rejected by netdata.cloud!</b>";
            return;
          }
      ...
      NETDATA.registry.delete(deleteRegistryUrl, function(result) {
        if (result !== null) {
          ...
          responseEl.innerHTML =
            "<b>Sorry, this command was rejected by the registry server!</b>";
        }
      });
    }
  }
...
var netdataDashboard = {
  ...
  sparkline: function(prefix, chart, dimension, units, suffix) {
    ...
    if (
      typeof options.data.charts[chart].dimensions[dimension] === "undefined"
    ) {
      return "";
    }
    ...
    return (
      prefix +
      '<div class="netdata-container" data-netdata="' +
      chart +
      '" data-after="-120" data-width="25%" data-height="15px" data-chart-library="dygraph" data-dygraph-theme="sparkline" data-dimensions="' +
      dimension +
      '" data-show-value-of-' +
      dimension +
      '-at="' +
      ...
      '"></div> (<span id="' +
      key +
      '" style="display: inline-block; min-width: 50px; text-align: right;">X</span>' +
      units +
      ")" +
      suffix
    );
  ...
  gaugeChart: function(title, width, dimensions, colors) {
    ...
    return (
      '<div class="netdata-container" data-netdata="CHART_UNIQUE_ID"' +
      ' data-dimensions="' +
      dimensions +
      '"' +
  ...
  menuTitle: function(chart) {
    if (typeof chart.menu_pattern !== "undefined") {
      return (
        ...
        "&nbsp;" +
        chart.type
          .slice(-(chart.type.length - chart.menu_pattern.length - 1))
          .toString()
      ).replace(/_/g, " ");
    }

    return this.anyAttribute(this.menu, "title", chart.menu, chart.menu)
      .toString()
      .replace(/_/g, " ");
  },
  ...
  submenuTitle: function(menu, submenu) {
    ...
    // console.log(key);
    var title = this.anyAttribute(this.submenu, "title", key, submenu)
      .toString()
      .replace(/_/g, " ");
    if (title.length > 28) {
  ...
  contextInfo: function(id) {
    ...
    if (x !== null) {
      return (
        '<div class="shorten dashboard-context-info netdata-chart-alignment" role="document">' +
        x +
        "</div>"
      );
    } else {
  ...
  contextValueRange: function(id) {
    if (
      typeof this.context[id] !== "undefined" &&
  ...
  contextHeight: function(id, def) {
    if (
      typeof this.context[id] !== "undefined" &&
      typeof this.context[id].height !== "undefined"
    ) {
...
function enrichChartData(chart) {
  ...
  switch (tmp) {
    ...
    case "cgroup":
      chart.menu = chart.type;
      if (
        chart.id.match(/.*[\._\/-:]qemu[\._\/-:]*/) ||
        chart.id.match(/.*[\._\/-:]kvm[\._\/-:]*/)
      ) {
    ...
    case "tc":
      ...
      if (
        chart.context === "tc.qos" &&
        (typeof options.submenu_names[chart.family] === "undefined" ||
          options.submenu_names[chart.family] === chart.family)
      ) {
        var n = chart.name.split(".")[1];
        if (n.endsWith("_in")) {
          options.submenu_names[chart.family] = n.slice(
            0,
            n.lastIndexOf("_in")
          );
...
function headMain(os, charts, duration) {
  ...
  if (typeof charts["system.swap"] !== "undefined") {
    head +=
      '<div class="netdata-container" style="margin-right: 10px;" data-netdata="system.swap"' +
  ...
  if (typeof charts["system.io"] !== "undefined") {
    ...
    head +=
      '<div class="netdata-container" style="margin-right: 10px;" data-netdata="system.pgpgio"' +
      ' data-dimensions="in"' +
      ' data-chart-library="easypiechart"' +
      ' data-title="Disk Read"' +
      ' data-width="11%"' +
  ...
  if (typeof charts["system.net"] !== "undefined") {
    head +=
      '<div class="netdata-container" style="margin-right: 10px;" data-netdata="system.net"' +
    ...
    head +=
      '<div class="netdata-container" style="margin-right: 10px;" data-netdata="system.ip"' +
      ' data-dimensions="received"' +
      ' data-chart-library="easypiechart"' +
    ...
    head +=
      '<div class="netdata-container" style="margin-right: 10px;" data-netdata="system.ipv6"' +
      ' data-dimensions="received"' +
      ' data-chart-library="easypiechart"' +
...
function generateHeadCharts(type, chart, duration) {
  ...
  var head = "";
  var hcharts = netdataDashboard.anyAttribute(
  ...
  if (hcharts.length > 0) {
    ...
    while (hi < hlen) {
      if (typeof hcharts[hi] === "function") {
        head += hcharts[hi](netdataDashboard.os, chart.id)
...
function renderPage(menus, data) {
  ...
  var duration =
    Math.round(
      ((($(div).width() * pcent_width) / 100) * data.update_every) / 3 / 60
    ) * 60;
  ...
  var html = "";
  var sidebar =
    '<ul class="nav dashboard-sidenav" data-spy="affix" id="sidebar_ul">';
  ...
  while (i < len) {
    ...
    sidebar +=
      ...
      '</a><ul class="nav">';
    html +=
      '<div role="section" class="dashboard-section"><div role="sectionhead"><h1 id="' +
      menuid +
      '" role="heading">' +
      menus[menu].icon +
      " " +
      menus[menu].title +
      '</h1></div><div role="section"  class="dashboard-subsection">';
    ...
    while (si < slen) {
      ...
      sidebar +=
        ...
        "</a></li>";
      shtml +=
        '<div role="section" class="dashboard-section-container" id="' +
        submenuid +
        '"><h2 id="' +
        submenuid +
        '" class="netdata-chart-alignment" role="heading">' +
        menus[menu].submenus[submenu].title +
        "</h2>";

      if (menus[menu].submenus[submenu].info !== null) {
        shtml +=
          '<div class="dashboard-submenu-info netdata-chart-alignment" role="document">' +
      ...
      var ci = 0,
        clen = menus[menu].submenus[submenu].charts.length;
      while (ci < clen) {
        ...
        function chartCommonMin(family, context, units) {
          var x = netdataDashboard.anyAttribute(
            netdataDashboard.context,
            "commonMin",
            context,
            undefined
          ...
          if (typeof x !== "undefined") {
            return (
              ' data-common-min="' + family + "/" + context + "/" + units + '"'
            );
          } else {
            return "";
          }
        ...
        function chartCommonMax(family, context, units) {
          ...
          if (typeof x !== "undefined") {
            return (
              ' data-common-max="' + family + "/" + context + "/" + units + '"'
            );
          } else {
            return "";
          }
        ...
        chtml +=
          '<div class="netdata-chartblock-container" style="width: ' +
          pcent_width.toString() +
          '%;">' +
          netdataDashboard.contextInfo(chart.context) +
          '<div class="netdata-container" id="chart_' +
          NETDATA.name2id(chart.id) +
          '" data-netdata="' +
          chart.id +
          '"' +
          ' data-width="100%"' +
          ' data-height="' +
          netdataDashboard
            .contextHeight(chart.context, options.chartsHeight)
            .toString() +
          'px"' +
          ' data-dygraph-valuerange="' +
          netdataDashboard.contextValueRange(chart.context) +
          '"' +
          ' data-before="0"' +
          ' data-after="-' +
          duration.toString() +
          '"' +
          ' data-id="' +
          NETDATA.name2id(options.hostname + "/" + chart.id) +
          '"' +
          ' data-colors="' +
          ...
          '"' +
          ' data-decimal-digits="' +
          netdataDashboard.contextDecimalDigits(chart.context, -1) +
          '"' +
  ...
  sidebar +=
    '<li class="" style="padding-top:15px;"><a href="https://github.com/netdata/netdata/blob/master/docs/Add-more-charts-to-netdata.md#add-more-charts-to-netdata" target="_blank"><i class="fas fa-plus"></i> add more charts</a></li>';
  sidebar +=
    '<li class=""><a href="https://github.com/netdata/netdata/tree/master/health#Health-monitoring" target="_blank"><i class="fas fa-plus"></i> add more alarms</a></li>';
  sidebar +=
    '<li class="" style="margin:20px;color:#666;"><small>netdata on <b>' +
    data.hostname.toString() +
    "</b>, collects every " +
    (data.update_every === 1
      ? "second"
      : data.update_every.toString() + " seconds") +
    " <b>" +
    data.dimensions_count.toLocaleString() +
    "</b> metrics, presented as <b>" +
    data.charts_count.toLocaleString() +
    "</b> charts and monitored by <b>" +
    data.alarms_count.toLocaleString() +
    "</b> alarms, using " +
    Math.round(data.rrd_memory_bytes / 1024 / 1024).toLocaleString() +
    " MB of memory for " +
  ...
  if (urlOptions.highlight === true) {
    NETDATA.globalChartUnderlay.init(
      null,
...
function renderChartsAndMenu(data) {
  ...
  for (var c in charts) {
    ...
    menu_key =
      typeof menus[m].menu_pattern !== "undefined" ? menus[m].menu_pattern : m;
    ...
    if (typeof menus[m].submenus[chart.submenu] === "undefined") {
      menus[m].submenus[chart.submenu] = {
        ...
        height: netdataDashboard.submenuHeight(
          menu_key,
          chart.submenu,
          menus[m].height
        )
  ...
  for (var m in menus) {
    ...
    for (var s in menus[m].submenus) {
      ...
      if (typeof options.submenu_names[s] !== "undefined") {
        ...
        menu_key =
          typeof menus[m].menu_pattern !== "undefined"
            ? menus[m].menu_pattern
            : m;
...
function clipboardCopyBadgeEmbed(url) {
  clipboard.writeText(
    '<embed src="' + url + '" type="image/svg+xml" height="20"/>'
  );
}
  ...
  NETDATA.alarms.get("all", function(data) {
    ...
    if (data === null) {
      document.getElementById(
        ...
        "alarms_all"
      ).innerHTML = document.getElementById("alarms_log").innerHTML =
    ...
    function alarm_lookup_explain(alarm, chart) {
      ...
      if (typeof alarm.lookup_dimensions !== "undefined") {
        ...
        if (x.length > 1) {
          dimensions =
            "of the sum of dimensions <code>" +
            alarm.lookup_dimensions +
            "</code> ";
        } else {
          dimensions =
            "of all values of dimension <code>" +
            alarm.lookup_dimensions +
            "</code> ";
        }
      }
      ...
      return (
        ...
        "</code>" +
        ", starting <code>" +
        NETDATA.seconds4human(alarm.lookup_after + alarm.lookup_before, {
          space: "&nbsp;"
        }) +
        "</code> and up to <code>" +
        NETDATA.seconds4human(alarm.lookup_before, { space: "&nbsp;" }) +
        "</code>" +
        (alarm.lookup_options
          ? ", with options <code>" +
            alarm.lookup_options.replace(/ /g, ",&nbsp;") +
            "</code>"
          : "") +
        "."
      );
    ...
    function alarm_to_html(alarm, full) {
      ...
      var has_alarm =
        typeof alarm.warn !== "undefined" || typeof alarm.crit !== "undefined";
      var badge_url =
        NETDATA.alarms.server +
        "/api/v1/badge.svg?chart=" +
        alarm.chart +
        "&alarm=" +
        alarm.name +
        "&refresh=auto";
      ...
      var action_buttons =
        ...
        alarm.recipient +
        "</b><br/>&nbsp;<br/>" +
        '<div class="action-button ripple" title="click to scroll the dashboard to the chart of this alarm" data-toggle="tooltip" data-placement="bottom" onClick="scrollToChartAfterHidingModal(\'' +
        alarm.chart +
        "'); $('#alarmsModal').modal('hide'); return false;\"><i class=\"fab fa-periscope\"></i></div>" +
        '<div class="action-button ripple" title="click to copy to the clipboard the URL of this badge" data-toggle="tooltip" data-placement="bottom" onClick="clipboardCopy(\'' +
        badge_url +
        '\'); return false;"><i class="far fa-copy"></i></div>' +
        '<div class="action-button ripple" title="click to copy to the clipboard an auto-refreshing <code>embed</code> html element for this badge" data-toggle="tooltip" data-placement="bottom" onClick="clipboardCopyBadgeEmbed(\'' +
        badge_url +
        '\'); return false;"><i class="fas fa-copy"></i></div>';

      var html =
        '<tr><td class="text-center" style="vertical-align:middle" width="40%"><b>' +
        alarm.chart +
        '</b><br/>&nbsp;<br/><embed src="' +
        badge_url +
        '" type="image/svg+xml" height="20"/><br/>&nbsp;<br/><span style="font-size: 18px">' +
        alarm.info +
        "</span>" +
        action_buttons +
        "</td>" +
        ...
        (typeof alarm.warn !== "undefined"
          ? '<tr><td width="10%" style="text-align:right">warning&nbsp;when</td><td><span style="font-family: monospace; color:#fe7d37; font-weight: bold;">' +
            alarm.warn +
            "</span></td></tr>"
          : "") +
        (typeof alarm.crit !== "undefined"
          ? '<tr><td width="10%" style="text-align:right">critical&nbsp;when</td><td><span style="font-family: monospace; color: #e05d44; font-weight: bold;">' +
            alarm.crit +
            "</span></td></tr>"
          : "");
      ...
      if (full === true) {
        ...
        html +=
          (typeof alarm.lookup_after !== "undefined"
            ? '<tr><td width="10%" style="text-align:right">db&nbsp;lookup</td><td>' +
              alarm_lookup_explain(alarm, chart) +
              "</td></tr>"
            : "") +
          (typeof alarm.calc !== "undefined"
            ? '<tr><td width="10%" style="text-align:right">calculation</td><td><span style="font-family: monospace;">' +
              alarm.calc +
              "</span></td></tr>"
            : "") +
          (chart.green !== null
            ? '<tr><td width="10%" style="text-align:right">green&nbsp;threshold</td><td><code>' +
              chart.green +
              " " +
              units +
              "</code></td></tr>"
            : "") +
          (chart.red !== null
            ? '<tr><td width="10%" style="text-align:right">red&nbsp;threshold</td><td><code>' +
              chart.red +
              " " +
              units +
              "</code></td></tr>"
      ...
      var delay = "";
      if (
        (alarm.delay_up_duration > 0 || alarm.delay_down_duration > 0) &&
        alarm.delay_multiplier !== 0 &&
        alarm.delay_max_duration > 0
      ) {
        if (alarm.delay_up_duration === alarm.delay_down_duration) {
          delay +=
            "<small><br/>hysteresis " +
            NETDATA.seconds4human(alarm.delay_up_duration, {
              space: "&nbsp;",
              negative_suffix: ""
            });
        } else {
          ...
          if (alarm.delay_down_duration > 0) {
            delay +=
              "on&nbsp;recovery&nbsp;<code>" +
              NETDATA.seconds4human(alarm.delay_down_duration, {
                space: "&nbsp;",
                negative_suffix: ""
              }) +
              "</code>, ";
        ...
        if (alarm.delay_multiplier !== 1.0) {
          delay +=
            "multiplied&nbsp;by&nbsp;<code>" +
            alarm.delay_multiplier.toString() +
            "</code>";
          delay +=
            ",&nbsp;up&nbsp;to&nbsp;<code>" +
            NETDATA.seconds4human(alarm.delay_max_duration, {
              space: "&nbsp;",
              negative_suffix: ""
            }) +
            "</code>";
      ...
      html +=
        '<tr><td width="10%" style="text-align:right">check&nbsp;every</td><td>' +
        NETDATA.seconds4human(alarm.update_every, {
          space: "&nbsp;",
          negative_suffix: ""
        }) +
        "</td></tr>" +
        (has_alarm === true
          ? '<tr><td width="10%" style="text-align:right">execute</td><td><span style="font-family: monospace;">' +
            alarm.exec +
            "</span>" +
            delay +
            "</td></tr>"
          : "") +
        '<tr><td width="10%" style="text-align:right">source</td><td><span style="font-family: monospace;">' +
        alarm.source +
        "</span></td></tr>" +
        "</table></td></tr>";
      ...
      if (typeof chart === "undefined") {
        console.log(
          "WARNING: alarm " +
            ...
            " is linked to chart " +
            alarm.chart +
            ", which is not found in the list of chart got from the server."
        );
        chart = { priority: 9999999 };
      } else if (
        typeof chart.menu !== "undefined" &&
        typeof chart.submenu !== "undefined"
      ) {
    ...
    while (len--) {
      ...
      all +=
        '<div class="panel panel-default"><div class="panel-heading" role="tab" id="alarm_all_heading_' +
        fc.toString() +
        '"><h4 class="panel-title"><a ' +
        collapsed +
        ' role="button" data-toggle="collapse" data-parent="#alarms_all_accordion" href="#alarm_all_' +
        fc.toString() +
        '" aria-expanded="' +
        expanded +
        '" aria-controls="alarm_all_' +
        fc.toString() +
        '">' +
        family.toString() +
        '</a></h4></div><div id="alarm_all_' +
        fc.toString() +
        '" class="panel-collapse collapse ' +
        cin +
        '" role="tabpanel" aria-labelledby="alarm_all_heading_' +
        fc.toString() +
        '" data-alarm-id="' +
        fc.toString() +
        '"><div class="panel-body" id="alarm_all_body_' +
        fc.toString() +
        '">';
      ...
      while (c--) {
        ...
        if (alarm.status === "WARNING" || alarm.status === "CRITICAL") {
          if (!active_family_added) {
            ...
            active +=
              '<tr><th class="text-center" colspan="2"><h4>' +
              family +
              "</h4></th></tr>";
          }
          count_active++;
    ...
    if (!count_active) {
      active +=
        '<div style="width:100%; height: 100px; text-align: center;"><span style="font-size: 50px;"><i class="fas fa-thumbs-up"></i></span><br/>Everything is normal. No raised alarms.</div>';
    } else {
      active += footer;
    }
    ...
    document.getElementById("alarms_log").innerHTML =
      '<h3>Alarm Log</h3><table id="alarms_log_table"></table>';
      ...
      $("#alarms_log_table").bootstrapTable({
        ...
        columns: [
          ...
          },
          {
            ...
            titleTooltip:
              "The incremental ID of this event for the given alarm",
            formatter: function(value, row, index) {
              void row;
              void index;
              return alarmid4human(value);
          ...
          },
          {
            ...
            title: "Status",
            titleTooltip:
              "The status of the alarm, that was set due to this event",
            align: "center",
            valign: "middle",
            switchable: false,
            sortable: true
          },
          {
            ...
            title: "Last Duration",
            titleTooltip:
              "The duration the alarm was at its previous state, just before this event",
            formatter: function(value, row, index) {
              void row;
              void index;
              return NETDATA.seconds4human(value, {
                negative_suffix: "",
                space: " ",
                now: "no time"
              });
          ...
          },
          {
            ...
            titleTooltip:
              "The duration the alarm was raised, just before this event",
            formatter: function(value, row, index) {
              void row;
              void index;
              return NETDATA.seconds4human(value, {
          ...
          },
          {
            ...
            titleTooltip:
              "True when this event has been updated by another event",
            formatter: function(value, row, index) {
              void row;
              void index;
          ...
          },
          {
            ...
            title: "Updates ID",
            titleTooltip:
              "The unique ID of the event obsoleted because of this event",
            formatter: function(value, row, index) {
              void row;
              void index;
              return alarmid4human(value);
          ...
          },
          {
            ...
            titleTooltip:
              "The date and time the script should be run, after hysteresis",
            formatter: function(value, row, index) {
              void row;
              void index;
              return timestamp4human(value, " ");
...
function initializeDynamicDashboardWithData(data) {
  if (data !== null) {
    ...
    document.getElementById("hostname").innerHTML =
      options.hostname +
      (netdataSnapshotData !== null ? " (snap)" : "").toString() +
      '&nbsp;&nbsp;<strong class="caret">';
...
function initializeDynamicDashboard(netdata_url) {
  ...
  NETDATA.pause(function() {
    if (typeof netdataCheckXSS !== "undefined" && netdataCheckXSS === true) {
      //$("#loadOverlay").css("display","none");
      document.getElementById("netdataXssModalServer").innerText =
        initializeConfig.url;
      $("#xssModal").modal("show");
    } else {
      initializeChartsAndCustomInfo();
    }
...
function getGithubLatestVersion(callback) {
  ...
  $.ajax({
    ...
    .fail(function() {
      versionLog(
        "Failed to download the latest stable version id from github!"
      );
      callback(null);
    });
}
    ...
    .done(function(response) {
      $.ajax({
        ...
        .fail(function(xhr, textStatus, errorThrown) {
          versionLog(
            "Failed to download the latest nightly version id from GCS!"
          );
          callback(null);
        });
    })
...
function notifyForUpdate(force) {
  ...
  checkForUpdateByVersion(force, function(sha1, sha2) {
    ...
    if (sha1 === null) {
      save = false;
      versionLog(
        '<p><big>Failed to get your netdata version!</big></p><p>You can always get the latest netdata from <a href="https://github.com/netdata/netdata" target="_blank">its github page</a>.</p>'
      );
    } else if (sha2 === null) {
      save = false;
      versionLog(
        '<p><big>Failed to get the latest netdata version.</big></p><p>You can always get the latest netdata from <a href="https://github.com/netdata/netdata" target="_blank">its github page</a>.</p>'
      );
    } else if (versionsMatch(sha1, sha2)) {
      save = true;
      versionLog(
        '<p><big>You already have the latest netdata!</big></p><p>No update yet?<br/>We probably need some motivation to keep going on!</p><p>If you haven\'t already, <a href="https://github.com/netdata/netdata" target="_blank">give netdata a <b><i class="fas fa-star"></i></b> at its github page</a>.</p>'
      );
    } else {
      save = true;
      var compare = "https://docs.netdata.cloud/changelog/";
      versionLog(
        "<p><big><strong>New version of netdata available!</strong></big></p><p>Latest version: <b><code>" +
          sha2 +
          '</code></b></p><p><a href="' +
          compare +
...
function printPreflight() {
  ...
  window.open(
    ...
    "width=" +
      ...
      ",height=" +
      height.toString() +
      ",menubar=no,toolbar=no,personalbar=no,location=no,resizable=no,scrollbars=yes,status=no,chrome=yes,centerscreen=yes,attention=yes,dialog=yes"
  );
...
function printPage() {
  ...
  $("#printModal").on("show.bs.modal", function() {
    ...
    function update_chart(idx) {
      ...
      var pcent =
        ((NETDATA.options.targets.length - idx) * 100) /
        NETDATA.options.targets.length;
      $(el)
        .css("width", pcent + "%")
        .attr("aria-valuenow", pcent);
...
function loadSnapshotModalLog(priority, msg) {
  document.getElementById("loadSnapshotStatus").className =
...
function loadSnapshot() {
  ...
  loadPako(function() {
    loadLzString(function() {
      ...
      if (
        typeof snapshotOptions.compressions[tmpSnapshotData.compression] ===
        "undefined"
      ) {
      ...
      tmpSnapshotData.uncompress =
        snapshotOptions.compressions[tmpSnapshotData.compression].uncompress;
      ...
      if (
        typeof tmpSnapshotData.highlight_after_ms !== "undefined" &&
  ...
  fr.onload = function(e) {
    ...
    try {
      ...
      document.getElementById("loadSnapshotHostname").innerHTML =
        ...
        result.hostname +
        "</b>, netdata version: <b>" +
        result.netdata_version.toString() +
        "</b>";
      document.getElementById("loadSnapshotURL").innerHTML = result.url;
      document.getElementById("loadSnapshotCharts").innerHTML =
        result.charts.charts_count.toString() +
        " charts, " +
        result.charts.dimensions_count.toString() +
        " dimensions, " +
        result.data_points.toString() +
        " points per dimension, " +
        Math.round(result.duration_ms / result.data_points).toString() +
        " ms per point";
      document.getElementById("loadSnapshotInfo").innerHTML =
        "version: <b>" +
        result.snapshot_version.toString() +
        "</b>, includes <b>" +
        result.charts_ok.toString() +
        "</b> unique chart data queries " +
        (result.charts_failed > 0
          ? "<b>" + result.charts_failed.toString() + "</b> failed"
          : ""
        ).toString() +
        ", compressed with <code>" +
        result.compression.toString() +
        "</code>, data size " +
        (Math.round((result.data_size * 100) / 1024 / 1024) / 100).toString() +
        " MB";
      document.getElementById("loadSnapshotTimeRange").innerHTML =
        "<b>" +
        NETDATA.dateTime.localeDateString(date_after) +
        " " +
        NETDATA.dateTime.localeTimeString(date_after) +
        "</b> to <b>" +
        NETDATA.dateTime.localeDateString(date_before) +
        " " +
        NETDATA.dateTime.localeTimeString(date_before) +
        "</b>";
      document.getElementById(
        "loadSnapshotComments"
      ).innerHTML = (result.comments ? result.comments : "").toString();
      loadSnapshotModalLog(
        "success",
        "File loaded, click <b>Import</b> to render it!"
      );
      ...
      document.getElementById("loadSnapshotStatus").className =
        "alert alert-danger";
      document.getElementById("loadSnapshotStatus").innerHTML =
...
function saveSnapshotModalLog(priority, msg) {
  document.getElementById("saveSnapshotStatus").className =
    "alert alert-" + priority;
...
function saveSnapshotModalShowExpectedSize() {
  var points = Math.round(
    saveSnapshotViewDuration / saveSnapshotSelectedSecondsPerPoint
  );
  var priority = "info";
  ...
  var sizemb =
    Math.round(
      ((options.data.charts_count * snapshotOptions.bytes_per_chart +
        options.data.dimensions_count *
          points *
          snapshotOptions.compressions[saveSnapshotCompression]
  ...
  saveSnapshotModalLog(
    priority,
    "The snapshot will have " +
      points.toString() +
      " points per dimension. Expected size on disk " +
      sizemb +
      " MB, at browser memory " +
      memmb +
      " MB.<br/>" +
      msg
  );
...
function saveSnapshotSetCompression(name) {
  ...
  document.getElementById(
    "saveSnapshotCompressionName"
  ).innerHTML = saveSnapshotCompression;
...
function saveSnapshotModalInit() {
  ...
  loadBootstrapSlider(function() {
    ...
    if (NETDATA.globalPanAndZoom.isActive() === true) {
      saveSnapshotViewDuration = Math.round(
    ...
    var yyyymmddhhssmm =
      ...
      NETDATA.zeropad(start_date.getMonth() + 1) +
      NETDATA.zeropad(start_date.getDate()) +
      "-" +
      NETDATA.zeropad(start_date.getHours()) +
      NETDATA.zeropad(start_date.getMinutes()) +
      NETDATA.zeropad(start_date.getSeconds());

    document.getElementById("saveSnapshotFilename").value =
      "netdata-" +
      options.hostname.toString() +
      "-" +
      yyyymmddhhssmm.toString() +
      "-" +
      saveSnapshotViewDuration.toString() +
      ".snapshot";
    ...
    var view = Math.round(
      saveSnapshotViewDuration /
        Math.round($(document.getElementById("charts_div")).width() / 2)
    );
...
function saveSnapshot() {
  loadPako(function() {
    loadLzString(function() {
      ...
      saveSnapshotModalLog(
        "info",
        "Generating snapshot as <code>" + filename.toString() + "</code>"
      );
      ...
      var saveData = {
        ...
        comments: document
          .getElementById("saveSnapshotComments")
          .value.toString(),
      ...
      if (
        typeof snapshotOptions.compressions[saveData.compression] ===
        "undefined"
      ) {
      ...
      var compress =
        snapshotOptions.compressions[saveData.compression].compress;
      var compressed_length =
        snapshotOptions.compressions[saveData.compression].compressed_length;
      ...
      saveData.data_points = Math.round(
        (saveData.before_ms - saveData.after_ms) /
          (saveSnapshotSelectedSecondsPerPoint * 1000)
      );
      saveSnapshotModalLog(
        "info",
        "Generating snapshot with " +
          saveData.data_points.toString() +
      ...
      var info =
        " Resolution: <b>" +
        saveSnapshotSelectedSecondsPerPoint.toString() +
        (saveSnapshotSelectedSecondsPerPoint === 1
          ? " second "
          : " seconds "
        ).toString() +
        "per point</b>.";
      ...
      function update_chart(idx) {
        ...
        setTimeout(function() {
          ...
          state.updateChart(function(status, reason) {
            ...
            saveSnapshotModalLog(
              charts_failed ? "danger" : "info",
              "Generated snapshot data size <b>" +
                (Math.round((size * 100) / 1024 / 1024) / 100).toString() +
                " MB</b>. " +
                (charts_failed
                  ? charts_failed.toString() +
                    " charts have failed to be downloaded"
                  : ""
                ).toString() +
                info
            );
...
function dashboardSettingsSetup() {
  var update_options_modal = function() {
    ...
    var units_sync_option = function(option) {
      ...
      if (self.prop("checked") !== (NETDATA.getOption("units") === "auto")) {
        self.bootstrapToggle(
          NETDATA.getOption("units") === "auto" ? "on" : "off"
        );
      }
    ...
    var temp_sync_option = function(option) {
      ...
      if (
        self.prop("checked") !==
        (NETDATA.getOption("temperature") === "celsius")
      ) {
        self.bootstrapToggle(
          NETDATA.getOption("temperature") === "celsius" ? "on" : "off"
        );
      }
    };
    var timezone_sync_option = function(option) {
      ...
      document.getElementById("browser_timezone").innerText =
        NETDATA.options.browser_timezone;
      document.getElementById("server_timezone").innerText =
        NETDATA.options.server_timezone;
      document.getElementById("current_timezone").innerText =
        NETDATA.options.current.timezone === "default"
          ? "unset, using browser default"
          : NETDATA.options.current.timezone;
  ...
  $("#units_temp").change(function() {
    NETDATA.setOption(
      "temperature",
      $(this).prop("checked") ? "celsius" : "fahrenheit"
    );
  });
...
function scrollDashboardTo() {
  if (
    netdataSnapshotData !== null &&
    typeof netdataSnapshotData.hash !== "undefined"
  ) {
    ...
    .on("activate.bs.scrollspy", function(e) {
      ...
      if (modal_shown === false) {
        ...
        if (
          typeof hash === "string" &&
          hash.substring(0, 1) === "#" &&
          urlOptions.hash.startsWith(hash + "_submenu_") === false
        ) {
  ...
  $("#myNetdataDropdownParent")
    .on("show.bs.dropdown", function() {
      ...
      $(".registry_link").each(function(idx) {
        this.setAttribute(
          "href",
          this.getAttribute("href").replace(/#.*$/, hash)
        );
      });
...
function finalizePage() {
  ...
  if (netdataSnapshotData !== null) {
    NETDATA.globalPanAndZoom.setMaster(
      NETDATA.options.targets[0],
      netdataSnapshotData.after_ms,
      netdataSnapshotData.before_ms
    );
  }
...
var selected_server_timezone = function(timezone, status) {
  ...
  if (typeof status === "undefined") {
    ...
    if (NETDATA.dateTime.init(timezone) === false) {
      ...
      document.getElementById("timezone_error_message").innerHTML =
        "Ooops! That timezone was not accepted by your browser. Please open a github issue to help us fix it.";
      NETDATA.setOption(
    ...
    if (NETDATA.options.current.user_set_server_timezone === "default") {
      NETDATA.options.current.user_set_server_timezone =
        NETDATA.options.server_timezone;
    }
    ...
    if (NETDATA.dateTime.init(timezone) === false) {
      ...
      document.getElementById("timezone_error_message").innerHTML =
        'Sorry. The timezone "' +
        timezone.toString() +
        '" is not accepted by your browser. Please select one from the list.';
      NETDATA.setOption(
        "user_set_server_timezone",
        NETDATA.options.server_timezone
      );
...
function truncateString(str, maxLength) {
  ...
  return `${str.substring(0, spanLength)}...${str.substring(
    str.length - spanLength
  )}`;
}
...
function deleteCloudAgentURL(agentID, url) {
  ...
  return fetch(
    ...
    }/api/v1/accounts/${cloudAccountID}/agents/${agentID}/url?value=${encodeURIComponent(
      url
    )}`,
    {
      method: "DELETE",
...
function signIn() {
  const url = `${NETDATA.registry.cloudBaseURL}/account/sign-in-agent?id=${
    NETDATA.registry.machine_guid
  }&name=${encodeURIComponent(
    NETDATA.registry.hostname
...
function renderAccountUI() {
  ...
  if (isSignedIn()) {
    ...
    container.innerHTML = `<a href="#" class="dropdown-toggle" data-toggle="dropdown"><span id="amc-account-name"></span> <strong class="caret"></strong></a>
    ...
    // container.innerHTML = `<a href="#" class="btn sign-in-btn theme-${netdataTheme}" onclick="signInDidClick(event); return false">
  ...
  for (const lagent of local) {
    ...
    if (cagent) {
      for (const u of lagent.alternate_urls) {
        if (u === NETDATA.registry.MASKED_DATA) {
          // TODO: temp until registry is updated.
          continue;
        }
...
function showSyncModal() {
  document.getElementById("sync-registry-modal-registry").innerHTML =
    NETDATA.registry.server;
...
function cloudSSOInit() {
  ...
  const url = `${NETDATA.registry.cloudBaseURL}/account/sso-agent?id=${
    NETDATA.registry.machine_guid
  }`;
  iframeEl.src = url;
